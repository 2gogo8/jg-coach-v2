...
const TRADER_TYPES: TraderType[] = [
  {
    ...
    advice: '你的優勢是「等到確認才進，進了就拿住」。建議只做趨勢明確的標的，不做盤整。進場條件：突破關鍵壓力區＋成交量放大。停損設在突破點以下7-10%，觸發立刻執行不等待。獲利設移動停損而非固定目標，讓趨勢決定出場時機。最忌諱的是在等待中失去耐心，提前在盤整中進場。',
    products: ['台指期波段單', 'AI概念股強勢波段', '台股電子權值股', '個股期貨趨勢追蹤'],
  },
  {
    ...
    advice: '你的優勢是「做到、做對、做滿」。建議建立固定選股清單，每週掃描，不在清單外亂動。每筆交易前先填完：進場理由、目標價、停損價——三者都填完才允許下單。最危險的時刻是你「感覺市場很好」，那時更要嚴格執行系統，不允許任何例外。',
    products: ['台股個股期貨（保證金明確）', '個股波段（固定週期評估）', '定期定額ETF', '台指期系統策略'],
  },
  {
    ...
    advice: '你的優勢是「感知到節奏比別人早」，但只在你最熟悉的標的上有效。建議只在2-3個最熟悉的標的上用直覺，其他標的改用系統。停損要設得比你直覺想的還緊——你進得快，也要出得快，不能讓「感覺還會回來」影響停損執行。',
    products: ['微台期短線', '熟悉個股快速波段', '小型股消息面交易', '當沖（限熟悉標的）'],
  },
  {
    ...
    advice: '你的優勢是「能在連輸之後繼續執行」。但你必須定期回測自己的系統，確認它還有正期望值。建議每季做一次覆盤，問自己：「這套系統在過去90天是否仍然有效？市場結構有沒有根本改變？」願意修正系統才是真正的系統信仰者。',
    products: ['台指期系統化交易', '選擇權固定策略（如賣PUT）', '量化規則選股', '回測驗證的波段策略'],
  },
  {
    ...
    advice: '你的優勢是「在別人還沒看到的時候就佈局完畢」。每個佈局必須設定「失效條件」——不是停損，而是「如果這件事沒發生，我就出場」。例如：「如果Q2財報沒有顯示成長加速，我出場」。這樣你不是被動停損，而是主動管理預期。',
    products: ['美股宏觀主題ETF', '台股產業輪動佈局', '定向選股（符合宏觀方向的個股）', '美股個股/ADR'],
  },
  {
    ...
    advice: '你的優勢是「在你的戰場上，你比任何人都更了解」。但你必須誠實問自己：這個戰場現在還是市場的主戰場嗎？建議每半年重新評估你聚焦的領域，如果市場資金已經轉向，你願意跟著轉嗎？護城河是能力，不是執念。',
    products: ['單一產業深度研究（半導體/AI/核能擇一精通）', '個股期貨（熟悉標的）', '台股海外雙掛牌', '該產業相關ETF'],
  },
  {
    ...
    advice: '你的優勢是「比市場早一步發現主題」。但主題有真假之分——真主題有財報支撐，假主題只有故事。進場前問自己：「這個主題有沒有可以驗證的財務指標？什麼時候可以看到數字？」有答案才進，沒有答案就是在賭故事。',
    products: ['美股AI/核能/生技主題先行者', '台股AI概念早期佈局', '主題ETF追蹤', '小型股爆發前段'],
  },
  {
    ...
    advice: '你的優勢是「永遠不會因為情緒破壞自己的系統」。你適合做「確定性高但報酬不是最高」的交易。建議設定清楚的資金分配比例：多少放確定性高的標的，多少放有爆發潛力的標的。不要讓守法的本性讓你完全錯過爆發機會。',
    products: ['台股定期定額優質個股', 'ETF配置為主（00878/006208）', '小比例資金測試高成長股', '個股期貨固定策略'],
  },
  {
    ...
    advice: '你的武器是耐心，但耐心和逃避停損是兩回事。確認你的每一個持倉都有「繼續持有的理由」，而不只是「不知道什麼時候該賣」。建議每季做一次持倉體檢：「這家公司的競爭力這一季有沒有變化？」理由消失就出場，這才是真正的長期主義，不是死抱。',
    products: ['台灣優質存股（護城河明確個股）', '美股S&P500 ETF長抱（VOO/SPY）', '高股息ETF複利（0056/00878）', '美股科技龍頭長抱（AAPL/MSFT）'],
  },
];
    ...
    return (
      <div style={{...S, padding:'32px 20px 80px'}}>
        <div id="result-content" style={{maxWidth:560,margin:'0 auto'}}>
          ...
          {/* Advice */}
          <div style={{background:'rgba(48,209,88,0.05)',border:'1px solid rgba(48,209,88,0.2)',borderRadius:20,padding:22,marginBottom:14}}>
            <div style={{fontSize:11,color:'#30D158',fontWeight:700,letterSpacing:'0.08em',marginBottom:10}}>📋 JG 給你的操作指引</div>
            <p style={{fontSize:14,color:'#EBEBF5CC',lineHeight:1.75,margin:'0 0 16px'}}>{resultType.advice}</p>
            <div style={{fontSize:11,color:'#30D158',fontWeight:700,letterSpacing:'0.08em',marginBottom:8}}>推薦嘗試的交易標的</div>
            <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
              {resultType.products.map((p, i) => (
                <span key={i} style={{background:'rgba(48,209,88,0.1)',color:'#30D158',border:'1px solid rgba(48,209,88,0.25)',borderRadius:10,padding:'4px 10px',fontSize:12,fontWeight:500}}>{p}</span>
              ))}
            </div>
          </div>
